...
/**
 * This {@link ImageCrawler} implementation strategy uses Java
 * parallel streams to perform an "image crawl" starting from a root
 * Uri.  Images from HTML page reachable from the root Uri are
 * downloaded from a remote web server or the local file system,
 * transformed, and then stored in files that can be displayed to the
 * user.  Likewise, image processing is either performed locally on
 * the client device or remotely using reactive microservices deployed
 * on one or more servers via Spring WebFlux.
 *
 * This implementation is a straightforward revision of the {@link
 * SequentialStreamsCrawler} class, with minor changes to use parallel
 * streams instead of sequential streams.
 */
...
public class ParallelStreamsCrawler // Loaded via reflection
    ...
    protected int crawlPage(String pageUri, int depth) {
        ...
        //    callInManagedBlocker() method to expand the Java common
        //    fork-join pool.
        //
