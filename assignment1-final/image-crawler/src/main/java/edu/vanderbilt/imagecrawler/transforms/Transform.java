...
/**
 * An abstract class that defines an interface for applying transformation
 * operations on an Image. Each Transform has a name and an abstract
...
public abstract class Transform {
	/**
	 * Supported transformations (with class as value to use 
	 * with newInstance() creation).
	 */
	public enum Type {
		GRAY_SCALE_TRANSFORM(GrayScaleTransform.class),
		TINT_TRANSFORM(TintTransform.class),
		SEPIA_TRANSFORM(SepiaTransform.class);
     ...
     * the transform.
     */
    public Image transform(Image image, Cache.Item item) {
        // Call the applyTransform() hook method.
        Image result = applyTransform(image, item);
        result.setTransformName(this);
        return result;
    }
	...
	public static final class Factory {
		...
		public static Transform newTransform(Type type, String name) {
			try {
				// Construct a new transform instance.
				Transform transform = type.mClazz.newInstance();
