...
public abstract class ImageCrawler
     ...
     */
    private BiFunction<InputStream,
                       Cache.Item,
                       PlatformImage> mNewImageFunction;
     ...
     */
    // @@ Monte, can this method return null?  If not, should we mark
    // it with the @NonNullable annotation?
     ...
     * @return A transformed {@link Image} or null if the transform
     * failed
     */
    @Nullable
     ...
     * cache doesn't already contain a matching item, addItem will
     ...
     * This method first checks the cache for an image that matches
     * the {@link URL} and if found returns that. Otherwise, it calls
     * the {@code managedBlockerDownloadImage()} helper method to
     * download the image and then returns the result.
     *
     * @param url The {@link URL} associated with an {@link Image}
     * @return The {@link Image} obtained from cache or by downloading
     *         or null if there was a problem downloading the {@link
     ...
     *         {@link Image} finishes being downloaded and stored in
     *         the cache
     */
