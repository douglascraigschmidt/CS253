...
class CompletableFuturesCrawlerTests : AssignmentTests(0) {
    ...
    @Test
    fun `crawlPageAsync() stops when maximum depth has been reached`() {
        val uri = "mock"
    ...
    fun `crawlPageAsync() build correct return value`() {
        ...
        every { mockCrawler.crawlHyperLinksOnPageAsync(mockFuture, -98) } returns mockIntFuture2
        ...
        verify(exactly = 1) {
            ...
            mockCrawler.crawlHyperLinksOnPageAsync(mockFuture, -98)
    ...
    fun `crawlHyperLinksOnPage() uses expected chain calls and lambdas`() {
        ...
        every { mockPage.getPageElementsAsStrings(any()) } answers {
