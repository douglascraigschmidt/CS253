...
class SequentialStreamsCrawlerTests : AssignmentTests() {
    ...
    private fun crawlPage(pages: Int, images: Int, failures: Boolean = false) {
        ...
        whenever(mockElements.stream()).thenReturn(elements.stream())
    ...
    private fun processImageTest(transforms: Int, failures: Int) {
        ...
        whenever(mockTransforms.stream()).thenReturn(transformList.stream())
        whenever(mockCrawler.getOrDownloadImage(url)).thenReturn(mockImage)
        whenever(mockCrawler.createNewCacheItem(any(), any())).thenReturn(true)
