...
class ParallelStreamsRemoteCrawlerTests : AssignmentTests(0) {
    ...
    fun `transformImageRemotely uses the expected chained method calls`() {
        ...
        every { mockTransformedImageList.stream() } answers { mockTransformedImageStream }
        ...
        verify(exactly = 1) {
            ...
            mockRemoteDataSource.applyTransforms(any(), mockImage, mockTransformNames)
            spyCrawler.remoteDataSource
            mockTransformedImageList.stream()
    ...
    fun `transformImageRemotely uses the expected lambdas`() {
        ...
        verify(exactly = 1) {
            spyCrawler.remoteDataSource
            spyRemoteDataSource.applyTransforms(any(), mockImage, mockTransformNames)
            spyCrawler.transformNames
        }
