...
class SequentialStreamsCrawlerTests : AssignmentTests() {
    ...
    fun `crawlPage() uses expected chained method calls and lambdas`() {
        ...
        confirmVerified(crawler, pc, si, mp, ps, hs, ss, wpc)
    }
    ...
    fun `processImage() uses expected chained method calls`() {
        ...
        confirmVerified(crawler, us, si, i, u)
    }
...
////        return Stream
////            .of(imageUrl)
////            .mapMulti((URL url, Consumer<Image> consumer) -> {
////            var image = getOrDownloadImage(url, this::downloadImage);
////            if (image != null)
...
//        every { crawler.downloadImage(any()) } throws AssertionError("Should not be called")
////        every { crawler.transformImage(any()) } throws AssertionError("Should not be called")
//
//        crawler.xxx(u)
////        crawler.processImage(u)
//
...
//    }
    ...
    fun `transformImage() uses expected chained method calls`() {
        ...
        val ic = mockk<Consumer<Image>>()
        val i = mockk<Image>()

        crawler.mTransforms = lt

        every { ic.accept(any()) } just Runs
        every { st.mapMulti(any<BiConsumer<Transform, Consumer<Image>>>()) } answers {
            firstArg<BiConsumer<Transform, Consumer<Image>>>().accept(mockk(), ic)
            si
        }
        ...
        verify(exactly = 1) {
            ...
            lt.stream()
            st.mapMulti(any<BiConsumer<Transform, Consumer<Image>>>())
        }
        confirmVerified(crawler, si, lt, st, ic)
    }
}