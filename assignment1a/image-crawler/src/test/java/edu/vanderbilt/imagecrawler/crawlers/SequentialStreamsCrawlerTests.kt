...
class SequentialStreamsCrawlerTests : AssignmentTests(0) {
    @SpyK
    ...
    fun before() {
        ...
        every { mockCrawler.log(any(), *anyVararg()) } answers { Unit }
    }
    ...
    @Test
    fun `performCrawl() should use expected chain method calls`() {
        ...
        every { mockOptional.orElse(0) } returns -99
        ...
        assertThat(mockCrawler.performCrawl(uri, 2)).isEqualTo(-99)

        verify { mockOptional.orElse(0) }
        verify { mockUriStream.map<Int>(any()) }
        verify { mockUriStream.filter(any()) }
        verify { Stream.of(uri) }
        verify { mockIntStream.findFirst() }
    }
    ...
    fun `performCrawl() ignores previously cached uris`() {
        ...
        verify { mockCrawler.mUniqueUris.putIfAbsent(uri) }
        confirmVerified(mockCrawler.mUniqueUris)
    }
    ...
    private fun processImageTest(transforms: Int, failures: Int) {
        ...
        every { mockCrawler.createNewCacheItem(any(), any<Transform>()) } returns true
        ...
        verify(exactly = transformList.size) { mockCrawler.createNewCacheItem(any(), any<Transform>()) }
