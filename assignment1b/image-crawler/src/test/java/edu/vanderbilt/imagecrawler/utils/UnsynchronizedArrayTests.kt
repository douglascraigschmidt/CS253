 ...
 */
class UnsynchronizedArrayTests : AssignmentTests(0) {
    /**
    ...
    fun `test remove() method`() {
        val input = shuffledMixedInput.toMutableList()
        val arraySpy = spyk(newArray(input))
        ...
        while (input.size > 0) {
            ...
            verify(exactly = 1) { arraySpy.rangeCheck(index) }
    ...
    fun `test get() method`() {
        ...
        verify(exactly = input.size) { arraySpy.rangeCheck(any()) }
    }
    ...
    fun `test set() method`() {
        ...
        for (i in input.indices) {
            ...
            assertSameContentsAndLength(input.toTypedArray(), getElements(arraySpy))
        }

        assertSameContentsAndLength(
                pool.toTypedArray(), getElements(arraySpy))

        // Check for the required number of rangeCheck calls.
        verify(exactly = pool.size) { arraySpy.rangeCheck(any()) }
    }
    ...
    fun `test add() method`() {
        ...
        val arraySpy = spyk(newArray(expected), recordPrivateCalls = true)

        assertSameContents(expected.toTypedArray(), arraySpy.toArray())
        ...
        for (i in 0..99) {
            ...
            val elements = getElements(arraySpy)
            assertSameContents(expected.toTypedArray(), elements)
            assertEquals(expected.size, arraySpy.size())
        }

        verify(exactly = addCalls) { arraySpy.ensureCapacityInternal(any()) }
    }
    ...
    private fun forEachBaseTest(block: (CustomArray<Any?>) -> Unit) {
        ...
        val spyArray = spyk(newArray(shuffledMixedInput))
        val result = newArray<Any?>()

        spyArray.forEach(Consumer { result.add(it) })

        // Ensure that a1 hasn't changed.
        assertArrayEquals(expected.toArray(), spyArray.toArray())

        // a2 should have the same contents and size as a1.
        assertArrayEquals(spyArray.toArray(), result.toArray())
    ...
    private fun <T> newArray(input: List<T>?) = UnsynchronizedArray(input)
    private fun <T> newArray() = UnsynchronizedArray<T>()
